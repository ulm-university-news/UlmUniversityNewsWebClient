...
public class LoadAccountsAction implements Action {
    /** An instance of the Logger class which performs logging for the LoadAccountsAction class. */
    private static final Logger logger = LoggerFactory.getLogger(LoadAccountsAction.class);

    /**
     * This method executes the business logic to load all accounts (moderator resources) from the server.
     *
    ...
    public String execute(RequestContextManager requestContext) throws SessionIsExpiredException, ServerException {
        ...
        if (requestContext.getRequestParameter("moderatorId") == null) {
            try {
                ...
                moderators = new ModeratorAPI().getModerators(activeModerator.getServerAccessToken(), false, null);
                ...
                return Constants.ACCOUNTS_LOAD_FAILED;
            }
        }

        return Constants.ACCOUNTS_LOADED;
    }
